...
class QuantAct(Module):
    ...
    def __init__(
        self, activation_bit, full_precision_flag=False, running_stat=True, beta=0.9
    ):
        """
    ...
    def forward(self, x):
        ...
        if self.running_stat:
            ...
            self.x_min = (self.x_min * self.beta + x_min * (1 - self.beta)) / (
                1 - self.beta_t
            )
            self.x_max = (self.x_max * self.beta + x_max * (1 - self.beta)) / (
                1 - self.beta_t
            )

        if not self.full_precision_flag:
            quant_act = self.act_function(
                x, self.activation_bit, self.x_min, self.x_max
            )
            return quant_act
        else:
            return x
...
class Quant_Embedding(Module):
    """
    Class to quantize given embedding layer weights
    """

    def __init__(self, weight_bit, full_precision_flag=False):
        """
        Args:
            weight_bit: bit-setting for activation
            full_precision_flag: full precision or not
        """
        ...
        self.embeding_dim = None
        # self.dtype = None
        self.weight = None
        self.padding_idx = None
        self.norm_type = None
        self.scale_grad_by_freq = None
        self.max_norm = None
        self.sparse = None

    def __repr__(self):
        s = super(Quant_Embedding, self).__repr__()
        s = (
            "("
            + s
            + " weight_bit{}, full_precision_flag={}".format(
